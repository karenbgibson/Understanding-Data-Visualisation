...
trimmed_books_with_range %>%  write.csv("books_cleaned.csv")
```

```{r}
(trimmed_range_summary <- trimmed_books_with_range %>% 
  group_by(page_range) %>% 
...
```{r}
top_5_years <- trimmed_books_with_range %>% 
...
```{r}
(trimmed_range_summary_two <- trimmed_books_with_range %>% 
  filter(publication_year %in% top_5_years$publication_year) %>% 
...
```{r}
total_books <- nrow(trimmed_books_with_range)

(page_range_overall <- trimmed_books_with_range %>% 
  group_by(page_range) %>% 
  summarise(num_books = n()) %>% 
  mutate(percentage = (num_books / total_books) * 100) %>%
...
```{r}
(page_range_top_500_review_count <- trimmed_books_with_range %>% 
...
page_range_overall + page_range_top_500_review_count
```
## Top 500 books based on average rating

```{r}
(page_range_top_500_avg_rating <- trimmed_books_with_range %>% 
...
```{r}
#pasting both graphs together for side by side view
page_range_overall + page_range_top_500_avg_rating
```